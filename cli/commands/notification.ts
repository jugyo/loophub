...
export async function run(): Promise<void> {
  ...
  if (sub === "send") {
    ...
    const sendUsage =
      "usage: lh notification send --kind implementation_done|over_budget|human_attention --title <text> --body <text|-> [--resource repo|issue:<n>|pull:<n>] [--herdr-pane-id <id>] [--source-key <key>] [--repo owner/name] [--json]";
