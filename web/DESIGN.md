...
| **Notification stack** | Application-wide stack of up to five newest unread notifications fixed above the status bar at the bottom-right of every route. It owns individual and bulk read actions, resource navigation, and live Herdr pane focus. | `NotificationStack` in [`src/components/notification-stack.tsx`](./src/components/notification-stack.tsx), composed by `AppLayout` |
| **Issue list** | Repository-scoped list and filtering page for issues. The repository top `/r/:owner/:repo` is the canonical (and only) issue list route. | `IssueList` in [`src/components/issue-list.tsx`](./src/components/issue-list.tsx), mounted by `routes/repo.tsx` |
...
- Shell regions and major route/page components (home, repo, issues, pulls,
  agents, stats, settings, workflows, and similar
  screens)
