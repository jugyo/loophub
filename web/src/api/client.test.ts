...
describe("rpc", () => {
  it("POSTs a JSON-RPC request to /rpc and returns the result", async () => {
    ...
    const data = await rpc<{ ok: boolean }>("repos/list", { archived: "active" });
    expect(data).toEqual({ ok: true });

    const { url, body } = lastRequest(fetchMock);
    expect(url).toBe("/rpc");
    expect(body).toMatchObject({ jsonrpc: "2.0", method: "repos/list", params: { archived: "active" } });
      ...
      vi.fn().mockResolvedValue(
        new Response(
          JSON.stringify({
            jsonrpc: "2.0",
            id: 1,
            error: { code: -32000, message: "Not Found", data: { status: 404 } },
          }),
          { status: 200, headers: { "content-type": "application/json" } },
        ),
      ),
    );
    await expect(rpc("issues/get", { repo: "me/proj", number: 9 })).rejects.toMatchObject({
      name: "ApiError",
      status: 404,
      message: "Not Found",
    });
  ...
  it("throws ApiError on a transport (non-2xx) failure", async () => {
    ...
    await expect(rpc("repos/list")).rejects.toMatchObject({ name: "ApiError", status: 502 });
...
describe("typed methods translate to contract params", () => {
  it("listRepos maps the REST archived flag to the contract enum", async () => {
    ...
    await listRepos("all");
    expect(lastRequest(fetchMock).body).toMatchObject({ method: "repos/list", params: { archived: "all" } });
  });

  it("listIssues parses the query string into structured params (labels -> array)", async () => {
    const fetchMock = mockRpc([]);
    await listIssues("me", "proj", "kind=issue&state=open&labels=bug,ui&per_page=20");
