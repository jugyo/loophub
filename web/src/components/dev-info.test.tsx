...
describe("IssueDevInfo", () => {
  it("shows lh build with the owner/repo/id, and hides the --sandbox variant", () => {
    ...
    expect(commands).toContain("lh build jugyo/loophub/150");
    // `--sandbox` is a hidden flag for now (Claude Code's macOS sandbox is broken — see #343);
    // the UI must not advertise it.
    expect(commands).not.toContain("lh build --sandbox jugyo/loophub/150");
  ...
  it("also shows the --auto (unattended) variant with a note, keeping the normal launch", () => {
    ...
    expect(commands).toContain("lh build jugyo/loophub/150");
    expect(commands).toContain("lh build --auto jugyo/loophub/150");
