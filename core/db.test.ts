...
test("bun test refuses to open the default LoopHub database", () => {
...
test(
  "workflowRunObservationTrail latency for an issue-list-sized run set",
  () => {
    D.db.run(
    ...
    expect(oldPlan.join(" | ")).not.toContain(
      "idx_events_repo_workflow_run_id",
    );
