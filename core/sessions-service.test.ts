...
test("sessions.list includes Cursor sessions without usage", () => {
  ...
  expect(list.some((s) => s.id === cursorSession)).toBe(true);
});
...
test("sessions.usageSync persists the Cursor chat id without inferring usage", async () => {
  const issue = svc.issues.create("me/proj", { title: "cursor usage" });
  const sessionId = "99999999-0000-0000-0000-0000000000c1";
  svc.sessions.register({
    id: sessionId,
    agent: "lh-build",
    session: sessionId,
  ...
  const cursorProjectsDir = mkdtempSync(join(tmpdir(), "lh-cursor-projects-"));
  const transcriptDir = join(
    cursorProjectsDir,
    SU.encodeCursorProjectCwd(realpathSync(worktree)),
    "agent-transcripts",
    "cursor-chat",
  );
  mkdirSync(transcriptDir, { recursive: true });
  writeFileSync(
    join(transcriptDir, "cursor-chat.jsonl"),
    JSON.stringify({
      role: "assistant",
      message: {
  ...
  ST.upsertSessionUsage(sessionId, {
    model: "cursor:auto",
    input_tokens: 100,
    cache_creation_input_tokens: 0,
  ...
  const swept = svc.sessions.usageSync({ cursorProjectsDir });
  expect(
    swept.sessions.find((row) => row.session_id === sessionId),
  ).toMatchObject({ status: "updated" });
  expect(getSession(sessionId).usage).toBeUndefined();
  expect(getSession(sessionId)).toMatchObject({
    session: "cursor-chat",
    model: "auto",
  });
  expect(
    svc.sessions.usageSync({ sessionId, cursorProjectsDir }),
  ).toMatchObject({ synced: 0, skipped: 1, missing: 0 });
  rmSync(cursorProjectsDir, { recursive: true, force: true });
});

test("sessions.usageSync correlates a Cursor parent from its successful launch time", async () => {
  const issue = svc.issues.create("me/proj", { title: "slow Cursor parent" });
  const sessionId = "99999999-0000-0000-0000-0000000000cb";
  ST.registerAgentSession(
    sessionId,
    "lh-workflow",
    sessionId,
    "slow Cursor parent",
  ...
  ST.setAgentSessionCreatedAt(sessionId, launchedAt);
  const cursorProjectsDir = mkdtempSync(join(tmpdir(), "lh-cursor-parent-"));
  const transcriptDir = join(
    cursorProjectsDir,
    SU.encodeCursorProjectCwd(realpathSync(worktree)),
    "agent-transcripts",
  ...
  mkdirSync(transcriptDir, { recursive: true });
  writeFileSync(
    join(transcriptDir, "cursor-parent-chat.jsonl"),
    JSON.stringify({
      role: "assistant",
  ...
  expect(
    svc.sessions.usageSync({ sessionId, cursorProjectsDir }),
  ).toMatchObject({ synced: 1, skipped: 0, missing: 0 });
  expect(getSession(sessionId)).toMatchObject({
    session: "cursor-parent-chat",
    model: "auto",
  });
  expect(ST.getAgentSession(sessionId)?.created_at).toBe(launchedAt);

  rmSync(cursorProjectsDir, { recursive: true, force: true });
});

test("sessions.usageSync does not correlate a late transcript to a failed Cursor launch", async () => {
  const issue = svc.issues.create("me/proj", { title: "failed cursor launch" });
  const sessionId = "99999999-0000-0000-0000-0000000000c8";
  svc.sessions.register({
    id: sessionId,
    agent: "workflow-step",
    session: sessionId,
  ...
  const worktree = join(HOME, "worktrees", "me", "proj", `pr-${opened.number}`);
  mkdirSync(worktree, { recursive: true });
  const cursorProjectsDir = mkdtempSync(join(tmpdir(), "lh-cursor-late-"));
  const transcriptDir = join(
    cursorProjectsDir,
    SU.encodeCursorProjectCwd(realpathSync(worktree)),
    "agent-transcripts",
  ...
  mkdirSync(transcriptDir, { recursive: true });
  writeFileSync(
    join(transcriptDir, "unrelated-late-chat.jsonl"),
    JSON.stringify({
      role: "assistant",
  ...
  expect(getSession(sessionId).session).toBe(sessionId);
  rmSync(cursorProjectsDir, { recursive: true, force: true });
});

test("sessions.usageSync pairs same-second Cursor sessions with transcripts in launch order", async () => {
  const issue = svc.issues.create("me/proj", {
    title: "concurrent cursor usage",
  });
  const firstSessionId = "99999999-0000-0000-0000-0000000000c4";
  const secondSessionId = "99999999-0000-0000-0000-0000000000c5";
  ...
  const worktree = join(HOME, "worktrees", "me", "proj", `pr-${opened.number}`);
  mkdirSync(worktree, { recursive: true });
  const cursorProjectsDir = mkdtempSync(join(tmpdir(), "lh-cursor-peers-"));
  const transcriptsDir = join(
    cursorProjectsDir,
    SU.encodeCursorProjectCwd(realpathSync(worktree)),
    "agent-transcripts",
  );
  for (const chatId of ["cursor-chat-a", "cursor-chat-b"]) {
    const transcriptDir = join(transcriptsDir, chatId);
    mkdirSync(transcriptDir, { recursive: true });
    writeFileSync(
  ...
  svc.sessions.usageSync({ sessionId: firstSessionId, cursorProjectsDir });

  expect(getSession(firstSessionId).session).toBe("cursor-chat-a");
  expect(getSession(firstSessionId).usage).toBeUndefined();
  expect(getSession(secondSessionId).session).toBe(secondSessionId);
  expect(getSession(secondSessionId).usage).toHaveLength(1);

  svc.sessions.usageSync({ cursorProjectsDir });

  expect(getSession(secondSessionId).session).toBe("cursor-chat-b");
  expect(getSession(secondSessionId).usage).toBeUndefined();
  rmSync(cursorProjectsDir, { recursive: true, force: true });
});

test("sessions.usageSync reserves a saved Cursor chat id before chronological matching", async () => {
  const issue = svc.issues.create("me/proj", {
    title: "known cursor chat",
  });
  const firstSessionId = "99999999-0000-0000-0000-0000000000c6";
  const knownSessionId = "99999999-0000-0000-0000-0000000000c7";
  for (const sessionId of [firstSessionId, knownSessionId]) {
    svc.sessions.register({
      id: sessionId,
      agent: "workflow-step",
      session: sessionId,
  ...
  ST.setAgentSessionExternalSession(knownSessionId, "cursor-known-chat-a");
  const createdAt = new Date(Date.now() - 1_000).toISOString();
  D.db.run(`UPDATE agent_sessions SET created_at = ? WHERE id IN (?, ?)`, [
    createdAt,
    firstSessionId,
  ...
  const worktree = join(HOME, "worktrees", "me", "proj", `pr-${opened.number}`);
  mkdirSync(worktree, { recursive: true });
  const cursorProjectsDir = mkdtempSync(join(tmpdir(), "lh-cursor-known-"));
  const transcriptsDir = join(
    cursorProjectsDir,
    SU.encodeCursorProjectCwd(realpathSync(worktree)),
    "agent-transcripts",
  );
  for (const chatId of ["cursor-known-chat-a", "cursor-known-chat-b"]) {
    const transcriptDir = join(transcriptsDir, chatId);
    mkdirSync(transcriptDir, { recursive: true });
    writeFileSync(
      join(transcriptDir, `${chatId}.jsonl`),
  ...
  expect(getSession(firstSessionId).session).toBe("cursor-known-chat-b");
  expect(getSession(knownSessionId).session).toBe("cursor-known-chat-a");
  rmSync(cursorProjectsDir, { recursive: true, force: true });
});

test("sessions.usageSync correlates an issue-create Cursor chat from the repository cwd", () => {
  const issue = svc.issues.create("me/proj", { title: "cursor issue create" });
  const sessionId = "99999999-0000-0000-0000-0000000000c2";
  svc.sessions.register({
    id: sessionId,
    agent: "issue-create",
    session: sessionId,
  ...
  svc.sessions.link("me/proj", { sessionId, issue: issue.number });

  const cursorProjectsDir = mkdtempSync(join(tmpdir(), "lh-cursor-projects-"));
  const transcriptDir = join(
    cursorProjectsDir,
    SU.encodeCursorProjectCwd(realpathSync(repoPath)),
    "agent-transcripts",
    "cursor-issue-chat",
  );
  mkdirSync(transcriptDir, { recursive: true });
  writeFileSync(
    join(transcriptDir, "cursor-issue-chat.jsonl"),
    JSON.stringify({
      role: "assistant",
      message: {
  ...
  expect(
    svc.sessions.usageSync({
      sessionId,
      cursorProjectsDir,
    }),
  ).toMatchObject({ synced: 1, skipped: 0, missing: 0 });
  expect(getSession(sessionId)).toMatchObject({
    session: "cursor-issue-chat",
    model: "composer-2.5",
  });
  expect(getSession(sessionId).usage).toBeUndefined();
...
test("sessions.usageSync claims issue-create Cursor chats per repository atomically", async () => {
  const cursorRepoPath = mkdtempSync(join(tmpdir(), "lh-cursor-issue-repo-"));
  const cursorGit = (args: string[]) =>
    spawnSync("git", ["-C", cursorRepoPath, ...args], { encoding: "utf8" });
  cursorGit(["init", "-q", "-b", "main"]);
  cursorGit(["config", "user.email", "t@t.local"]);
  cursorGit(["config", "user.name", "tester"]);
  writeFileSync(join(cursorRepoPath, "a.txt"), "x\n");
  cursorGit(["add", "-A"]);
  cursorGit(["commit", "-qm", "init"]);
  await svc.repos.create({
    path: cursorRepoPath,
    name: "me/cursor-issue-repo",
  });

  const firstIssue = svc.issues.create("me/cursor-issue-repo", {
    title: "first cursor issue create",
  });
  const secondIssue = svc.issues.create("me/cursor-issue-repo", {
    title: "second cursor issue create",
  });
  const firstSessionId = "66666666-0000-0000-0000-000000000001";
  const secondSessionId = "66666666-0000-0000-0000-000000000002";
  for (const [sessionId, issueNumber] of [
    [firstSessionId, firstIssue.number],
    [secondSessionId, secondIssue.number],
  ] as const) {
    svc.sessions.register({
      id: sessionId,
      agent: "issue-create",
    ...
    svc.sessions.link("me/cursor-issue-repo", {
      sessionId,
      issue: issueNumber,
    });
    ST.upsertSessionUsage(sessionId, {
  ...
  D.db.run(`UPDATE agent_sessions SET updated_at = ? WHERE id = ?`, [
  ...
  const cursorProjectsDir = mkdtempSync(join(tmpdir(), "lh-cursor-issues-"));
  const transcriptsDir = join(
    cursorProjectsDir,
    SU.encodeCursorProjectCwd(realpathSync(cursorRepoPath)),
    "agent-transcripts",
  );
  for (const chatId of ["cursor-issue-chat-a", "cursor-issue-chat-b"]) {
    const transcriptDir = join(transcriptsDir, chatId);
    mkdirSync(transcriptDir, { recursive: true });
    writeFileSync(
      join(transcriptDir, `${chatId}.jsonl`),
  ...
  D.db.exec(`
    CREATE TABLE cursor_issue_update_guard (count INTEGER NOT NULL);
    INSERT INTO cursor_issue_update_guard VALUES (0);
    CREATE TRIGGER fail_second_cursor_issue_update
    BEFORE UPDATE OF external_session ON agent_sessions
    BEGIN
      UPDATE cursor_issue_update_guard SET count = count + 1;
      SELECT CASE WHEN (SELECT count FROM cursor_issue_update_guard) = 2
        THEN RAISE(ABORT, 'injected Cursor issue update failure') END;
    END
  `);
  try {
    expect(() => svc.sessions.usageSync({ cursorProjectsDir })).toThrow(
      "injected Cursor issue update failure",
    );
    expect(getSession(firstSessionId).session).toBe(firstSessionId);
    expect(getSession(secondSessionId).session).toBe(secondSessionId);
    expect(getSession(firstSessionId).usage).toHaveLength(1);
    expect(getSession(secondSessionId).usage).toHaveLength(1);
  } finally {
    D.db.run("DROP TRIGGER fail_second_cursor_issue_update");
    D.db.run("DROP TABLE cursor_issue_update_guard");
  }

  const synced = svc.sessions.usageSync({ cursorProjectsDir });
  expect(
    synced.sessions
      .filter((row) =>
        [firstSessionId, secondSessionId].includes(row.session_id),
      )
      .map((row) => row.status),
  ).toEqual(["updated", "updated"]);
  expect(
    new Set([
      getSession(firstSessionId).session,
      getSession(secondSessionId).session,
    ]),
  ).toEqual(new Set(["cursor-issue-chat-a", "cursor-issue-chat-b"]));
  expect(getSession(firstSessionId).usage).toBeUndefined();
  expect(getSession(secondSessionId).usage).toBeUndefined();

  rmSync(cursorProjectsDir, { recursive: true, force: true });
  rmSync(cursorRepoPath, { recursive: true, force: true });
});

test("sessions.usageSync replaces Cursor usage atomically", async () => {
  const issue = svc.issues.create("me/proj", { title: "cursor atomic usage" });
  const sessionId = "99999999-0000-0000-0000-0000000000c3";
  svc.sessions.register({
    id: sessionId,
    agent: "lh-build",
    session: sessionId,
  ...
  const worktree = join(HOME, "worktrees", "me", "proj", `pr-${opened.number}`);
  mkdirSync(worktree, { recursive: true });
  const cursorProjectsDir = mkdtempSync(join(tmpdir(), "lh-cursor-atomic-"));
  const transcriptDir = join(
    cursorProjectsDir,
    SU.encodeCursorProjectCwd(realpathSync(worktree)),
    "agent-transcripts",
  ...
  mkdirSync(transcriptDir, { recursive: true });
  const transcript = join(transcriptDir, "cursor-atomic-chat.jsonl");
  writeFileSync(
    transcript,
    JSON.stringify({
      role: "assistant",
  ...
  svc.sessions.usageSync({ sessionId, cursorProjectsDir });
  ST.setAgentSessionExternalSession(sessionId, sessionId);
  ST.upsertSessionUsage(sessionId, {
    model: "cursor:auto",
    input_tokens: 100,
    cache_creation_input_tokens: 0,
  ...
  const eventCountBeforeFailure = (
    D.db.query("SELECT COUNT(*) AS count FROM events").get() as {
      count: number;
    }
  ).count;

  D.db.exec(
    `CREATE TRIGGER fail_cursor_usage_delete
     BEFORE DELETE ON session_usage
     WHEN OLD.session_id = '${sessionId}'
     BEGIN SELECT RAISE(ABORT, 'injected Cursor usage failure'); END`,
  );
  try {
    expect(() =>
      svc.sessions.usageSync({ sessionId, cursorProjectsDir, full: true }),
    ).toThrow("injected Cursor usage failure");
  } finally {
    D.db.run("DROP TRIGGER fail_cursor_usage_delete");
    rmSync(cursorProjectsDir, { recursive: true, force: true });
  }
