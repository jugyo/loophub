...
test("a GitHub-detected merge runs the same sibling cleanup", async () => {
  const repo = await makeRepo("me/github-attempts");
  const issue = svc.issues.create("me/github-attempts", {
    title: "choose remote",
  });
  branch(repo.path, "github-adopted");
  branch(repo.path, "github-sibling");
  const adopted = await createAttempt(
    "me/github-attempts",
    issue.number,
    "github-adopted",
  );
  const sibling = await createAttempt(
    "me/github-attempts",
    issue.number,
    "github-sibling",
  );
  const adoptedRow = S.getIssue(repo.id, adopted.number)!;
  S.recordGithubPull({
    issueId: adoptedRow.id,
    number: 9001,
    url: "https://github.com/me/github-attempts/pull/9001",
    createdBy: "tester",
  });
  S.setGithubMerged(adoptedRow.id, "2026-07-11T00:00:00Z");

  await svc.pulls.markGithubMerged(
    "me/github-attempts",
    adopted.number,
    "github-sync",
  );
  ...
  expect(
    (await svc.pulls.get("me/github-attempts", sibling.number)).state,
  ).toBe("closed");
  const siblingRow = S.getIssue(repo.id, sibling.number)!;
  expect(S.listComments(siblingRow.id)[0]?.body).toBe(
    `Superseded by #${adopted.number}.`,
  );
});
