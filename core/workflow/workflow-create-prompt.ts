...
workflow は固定の Execute/Verify 開発ループ用の global な prompt バンドルです。Execute / Verify のステップ contract は LoopHub が固定しており、設定できるのは各 contract の上にユーザーが書く追加的なステップごとの指示だけです。あなたはその指示を集めて workflow を作成するのが仕事であり、workflow の実行や実装そのものは決して行いません。
...
export function workflowCreatePrompt(language: unknown): string {
  return language === "ja"
    ? WORKFLOW_CREATE_PROMPTS.ja
    : WORKFLOW_CREATE_PROMPTS.en;
}
