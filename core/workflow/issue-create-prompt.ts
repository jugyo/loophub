...
Keep the issue body concise and easy to scan. Present the problem or goal first, then only the scope needed to understand the requested outcome, followed by related resources when any exist. Exclude implementation steps, excessive emphasis, and explanations that repeat the same information. Create acceptance criteria only as concise, verifiable structured criteria; do not duplicate them in the body. For detailed source material, include a brief summary and its reference instead of copying its contents into the body.
...
issue body は簡潔で読み取りやすくしてください。問題または goal、依頼の結果を理解するために必要な scope、関連リソースがある場合はその情報、の順に記述してください。実装手順、過度な強調、同じ情報の重複説明は含めないでください。acceptance criteria は簡潔で検証可能な structured criteria としてのみ作成し、body に重複させないでください。詳細な資料は、内容を body に転載せず、短い要約と参照先を示してください。
