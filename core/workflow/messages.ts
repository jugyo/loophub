...
const WORKFLOW_MESSAGE_CATALOG = {
  ...
  parentPrompt: {
    en: (input) => [
      ...
      "",
      "## Instruction",
      `Run \`lh events subscribe --repo ${input.repoArg} --target herdr-pane --session "$HERDR_SESSION" --pane "$HERDR_PANE_ID" --resource workflow_run:${input.runId} --resource issue:${input.issueNumber} --resource pull:${input.prNumber} --json\` first, before anything else, and keep the returned subscription id. This pane is woken by a ping only after that registration.`,
      "Then orchestrate this run through Execute -> Verify as described in your contract.",
      "Read the run's current state, reconcile from it, and wait for the next ping delivered to this pane, as described in the contract. Do not invoke slash-style commands.",
      "",
    ],
    ja: (input) => [
      "## Run コンテキスト",
      ...
      "",
      "## 指示",
      `まず最初に \`lh events subscribe --repo ${input.repoArg} --target herdr-pane --session "$HERDR_SESSION" --pane "$HERDR_PANE_ID" --resource workflow_run:${input.runId} --resource issue:${input.issueNumber} --resource pull:${input.prNumber} --json\` を実行し、返る subscription id を保持してください。この登録の後にのみ、この pane が ping で起こされます。`,
      "その上で、contract の記述に従い、この run を Execute -> Verify の順に orchestrate してください。",
      "contract に従って run の現在の state を読み、そこから reconcile し、この pane に届く次の ping を待ってください。slash-style command は呼び出さないでください。",
      "",
    ],
  },
