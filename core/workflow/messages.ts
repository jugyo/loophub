...
const WORKFLOW_MESSAGE_CATALOG = {
  ...
  parentPrompt: {
    en: (input) => [
      ...
      `Decide every transition by observing \`lh workflow step status ${input.runId} --repo ${input.repoArg} --json\` after the foreground watch returns an event; command completion, pane output, and PR body markers are never transition facts.`,
      "Start now:",
      ...
      `3. Run \`lh workflow watch --repo ${input.repoArg} --run ${input.runId} --since <cursor> --json\` in the foreground and remain blocked until it exits. Do not use a background-task option or deferred cell.`,
    ...
    ja: (input) => [
      ...
      `foreground watch が event を返した後、\`lh workflow step status ${input.runId} --repo ${input.repoArg} --json\` を観測して各遷移を判断してください。command completion、pane output、PR body marker は遷移の事実ではありません。`,
      "今すぐ開始してください:",
      ...
      `3. \`lh workflow watch --repo ${input.repoArg} --run ${input.runId} --since <cursor> --json\` を foreground で実行し、終了まで block します。background-task option や deferred cell を使いません。`,
      "4. 返された `events` array 内の単一 event を処理し、各遷移について step status を再観測し、<cursor> を `events[0].id` に更新して foreground watch を繰り返します。",
