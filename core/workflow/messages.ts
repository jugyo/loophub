...
const WORKFLOW_MESSAGE_CATALOG = {
  ...
  parentPrompt: {
    en: (input) => [
      ...
      "",
      "## Instruction",
      `Run \`lh workflow parent-ready ${input.runId} --repo ${input.repoArg}\` first, before anything else. Instructions are delivered to this pane only after that signal.`,
    ...
    ja: (input) => [
      ...
      "",
      "## 指示",
      `まず最初に \`lh workflow parent-ready ${input.runId} --repo ${input.repoArg}\` を実行してください。この signal の後にのみ instruction がこの pane に配送されます。`,
