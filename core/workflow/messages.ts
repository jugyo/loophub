...
const WORKFLOW_MESSAGE_CATALOG = {
  ...
  parentPrompt: {
    en: (input) => [
      ...
      `repo: ${input.repoName} (run-scoped lh workflow commands resolve the repo from --run; pass --repo ${input.repoArg} only on other lh commands)`,
      ...
      "",
      "## Instruction",
      `Run \`lh workflow parent-ready ${input.runId}\` first, before anything else. Instructions are delivered to this pane only after that signal.`,
    ...
    ja: (input) => [
      ...
      `run: ${input.runId}`,
      `workflow: ${input.workflowName}`,
      `repo: ${input.repoName} (--run を取る lh workflow command は run から repo を解決します。--repo ${input.repoArg} はそれ以外の lh command にだけ渡してください)`,
      `issue: #${input.issueNumber}`,
      `pr: #${input.prNumber}`,
      "current step: execute",
      `worktree: . (cwd。base branch: ${input.baseRef})`,
      "",
      "## 指示",
      `まず最初に \`lh workflow parent-ready ${input.runId}\` を実行してください。この signal の後にのみ instruction がこの pane に配送されます。`,
