...
const WORKFLOW_MESSAGE_CATALOG = {
  ...
  parentPrompt: {
    en: (input) => [
      ...
      `2. Start \`lh workflow next ${input.runId} --repo ${input.repoArg} --watch --json\` as a runtime-managed background task and end the model turn while it blocks. Resume only from the task completion notification, then read its JSON result. Runtime-specific tool mechanics belong to the runtime adapter, not this prompt.`,
    ...
    ja: (input) => [
      ...
      "各遷移は `lh workflow next` が返す `action` と `observed` から判断してください。task completion、pane output、PR body marker は遷移の事実ではありません。",
      "今すぐ開始してください:",
      `1. Execute child を起動します: \`lh workflow launch-step --repo ${input.repoArg} --run ${input.runId} --step execute\`.`,
      `2. \`lh workflow next ${input.runId} --repo ${input.repoArg} --watch --json\` を runtime-managed background task として開始し、block 中は model turn を終了します。task completion 通知だけを契機に再開し、JSON result を読みます。runtime 固有の tool mechanism はこの prompt ではなく runtime adapter の責務です。`,
      "3. task completion 後、返された action を contract の手順どおり実行し、同じ `next --watch` command を再度開始します。event の受信と再開位置はこの command が管理するため、cursor を seed・acknowledge しません。",
