...
const WORKFLOW_MESSAGE_CATALOG = {
  ...
  parentPrompt: {
    en: (input) => [
      ...
      `Decide every transition by observing \`lh workflow step status ${input.runId} --repo ${input.repoArg} --json\` after the watcher exec session returns an event; command completion, pane output, and PR body markers are never transition facts.`,
      "Start now:",
      ...
      `3. Start \`lh workflow watch --repo ${input.repoArg} --run ${input.runId} --since <cursor> --json\` with \`exec_command\`. If it returns a \`session_id\` before completion, wait with \`write_stdin\` using the same session, empty input, and a long yield. Do not emit a final parent response while waiting.`,
      "4. On completion, process the single event in the returned `events` array and re-observe step status for every transition. After processing it, pass the returned `next_command` unchanged to the next `exec_command`; do not reconstruct or edit its cursor.",
    ...
    ja: (input) => [
      ...
      `watcher exec session が event を返した後、\`lh workflow step status ${input.runId} --repo ${input.repoArg} --json\` を観測して各遷移を判断してください。command completion、pane output、PR body marker は遷移の事実ではありません。`,
      "今すぐ開始してください:",
      ...
      `3. \`lh workflow watch --repo ${input.repoArg} --run ${input.runId} --since <cursor> --json\` を \`exec_command\` で開始します。完了前に \`session_id\` が返った場合は、同じ session、空入力、長い yield の \`write_stdin\` で待ちます。待機中は parent の最終応答を出しません。`,
      "4. 完了後、返された `events` array 内の単一 event を処理し、各遷移について step status を再観測します。処理後は返された `next_command` を編集せず、そのまま次の `exec_command` に渡します。cursor を組み立て直しません。",
