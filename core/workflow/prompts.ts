...
export function parentUserPrompt(input: {
  ...
  return [
    ...
    `3. With \`HERDR_ENV=1\`, create \`\${LOOPHUB_HOME:-$HOME/.loophub}/logs/workflow-parent-watch\` and arm one detached watcher: \`nohup scripts/workflow-parent-watch.sh --repo ${repo} --run ${input.runId} --since "$cursor" --herdr-session "$HERDR_SESSION" --parent-pane "$HERDR_PANE_ID" >>"\${LOOPHUB_HOME:-$HOME/.loophub}/logs/workflow-parent-watch/run-${input.runId}.log" 2>&1 </dev/null &\`.`,
    "4. Set `watcher_armed=true` and end the model turn; only the shell watcher polls while waiting.",
