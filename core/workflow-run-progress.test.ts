...
describe("workflow run progress", () => {
  test("worktreeHead returns HEAD, and rejects when it cannot resolve", async () => {
    ...
    try {
      const error = await worktreeHead(nonRepo).catch((e: unknown) => e);
      expect(error).toSatisfy(
        (e: unknown) => isServiceError(e) && e.status === 422,
      );
    } finally {
  ...
  test("workflowRunProgress keeps an unresolvable base visible with collision hints (#39 AC-3)", async () => {
    ...
    const error = await workflowRunProgress({
      worktree: REPO,
    ...
    expect(error).toSatisfy(
      (e: unknown) =>
        isServiceError(e) &&
        e.status === 422 &&
        /could not resolve Workflow base branch 'missing-base'/.test(
          e.message,
        ) &&
  ...
  test("workflowRunProgress tolerates a shadowing $GIT_DIR/<base> pseudo-ref (#39)", async () => {
    ...
    expect(await pinnedBaseSha(REPO, "opencode", head)).toMatch(
      /^[0-9a-f]{40}$/,
    );
