...
> `lh-dev` / `skills/lh-dev` も廃止済み。本書内のこれらの名称・スラッシュコマンド・運用記述は、すべて
> 当時の設計を説明する歴史的参照であり、現行機能の案内ではない。
...
- **何**: 当時の `lh-dev` の後継案。開発を **Plan → Code → Test&Verify** に分け、各フェーズを **別々の使い捨て
...
> 当時は v1 を捨てず、小～中規模・低リスクの変更には v1、**本番級・要検証**の変更や AFK で長く回す
> タスクには v2、という使い分けを想定していた。

---

## 2. 名前

**当時の採用名: `lh-build`**（スラッシュコマンド `/lh-build`）。短くコマンド向きで、「検証済みの変更を
*build* する」を表す。`lh-dev`(v1) と併存する後継スキルとして設計され、本書では以後この名前で通す。
...
- **`lh-pr-review` / `lh-merge-ready`**: lh-build は **呼ばない**（自前の reviewer サブと最終ゲートで代替）。
  完全分離の理由と構造的担保は **§9.5**。
- **`lh-dev`(v1)**: 当時は併存を想定。v1 = 単一実装サブの軽量モード、lh-build = 3フェーズの重量モード。
