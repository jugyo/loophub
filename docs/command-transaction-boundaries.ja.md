...
| `prTestMaps.create` | procedure | test map row、`pull_request.test_map_created` | head SHA の git read |
...
| `diffFeedback.create` / `createHuman` | procedure | thread、first message、`pull_request.diff_feedback_created` | anchor 解決の git diff read |
