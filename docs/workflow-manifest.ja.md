...
> 子エージェントの起動はこの manifest を読んで行う。Execute / Verify の 2 step モデル、pointer 入力、
> HEAD / review 観測による遷移は変更しない。
...
## 3. 役割分担 — configuration と state

本設計の中心にある線引きはこれ 1 本である。

| 種別 | 内容 | 在処 | 書く主体 | 更新頻度 |
|---|---|---|---|---|
| **configuration（動作条件）** | runtime / model / effort / step prompt / 言語 / 参照する issue・PR・worktree | **manifest（ファイル）** | LoopHub（run 開始時）と人間（途中変更） | 稀。人間が明示的に編集したときだけ |
...
state をファイルに出さないのは、LoopHub の書き込み規約
...
逆に configuration は「人間が読んで編集する」対象であり、event と同時 commit する必要がない。したがって
ファイルが適している。
...
manifest は §3 の configuration だけを持ち、state は DB に残す。子の launch は manifest だけを読む。
...
```jsonc
{
  ...
  "run": {
    ...
    "issue": 94,
    "pr": 103,
    "workflow": { "id": 3, "name": "design" }
  },
  "workspace": {
    "worktree_path": "/Users/me/.loophub/worktrees/jugyo/loophub/pr-103",
    "branch": "loophub/pr-103",
    "base_branch": "main"
  },
...
| `run.*` / `workspace.*` | **pointer の複製（読み取り専用）**。どの run の manifest かを同定する | DB と不一致なら launch を可視エラーで止める（§5.4） |
...
`run.*` と `workspace.*` を「編集しても効かない読み取り専用」と決めるのは、worktree と branch が PR 番号
から導出される規約（[worktree ライフサイクル](worktree.ja.md)）だからである。manifest で worktree を
別の場所へ向けられるようにすると、その規約と `lh worktree prune` の前提が壊れる。issue が候補に挙げた
「issue id / pr id / worktree path / branch」は **記録として持つ**が、**設定としては持たない**。
...
`workflowRuns.launchStep()` は、現在の 4 つの解決関数（`runRuntime` / `runModel` /
`runContractLanguage` / `workflowStepPrompt`）をやめ、manifest から解決した 1 つの launch configuration を
使う。parent の launch（`lh workflow start` の後半）も同じ configuration の `agents.parent` を読む。

検証は読み込みのたびに行い、次のいずれかに当たれば **launch を非 0 で止める**。

1. `manifest.json` が無い / 読めない / JSON として不正
2. `manifest_version` が既知でない
3. `run.id` / `run.repo` / `run.issue` / `run.pr` が DB 行と一致しない（run ディレクトリの取り違え・複製の検出）
4. `agents.<kind>.runtime` が runtime registry に無い、または 3 つの `runtime` が一致しない（v1 の制約）
5. `agents.<kind>.model` が空
6. `prompts.<step>` が単純ファイル名でない、または参照先が読めない
7. 未知の key がある

7 を「無視」ではなく「エラー」にするのは意図的である。`core/workflow.ts` の `normalizeWorkflow()` は
...
lh workflow manifest show <run> [--repo <owner/name>] [--json]
    # manifest を検証して表示する。--json は解決済みの絶対 path と
    # 「どの変更がいつ効くか」を含む
...
| 2 | 置き場所は `$LOOPHUB_HOME/runs/workflow/<runId>/` | 既存 run file と同じ領域。repository worktree の外なので git に混入し得ない。symlink 硬化済み helper を再利用できる |
...
- **pointer 入力は変わらない。** Execute は (repo, issue, pr[, review])、Verify は
  (issue, base SHA, head SHA, 提出先 PR)。manifest はこの pointer 集合に何も足さない。
...
| リスク | 影響 | 緩和 |
|---|---|---|
| DB と manifest の drift（同じ値が 2 か所にある） | 表示と実挙動が食い違う | §3 の役割分割を本書と code comment で明文化する。**launch 経路は manifest しか読まない**という 1 方向の依存にする。DB 列は表示用の非正規化コピーと位置づける |
...
5. **manifest を run 間で再利用するか。** `lh workflow start --manifest <path>` のようなテンプレート起動は
   G7（LoopHub 本体との疎結合）の自然な次の一歩だが、pointer（run id / issue / pr）が run 固有なので
   「テンプレート部分」と「run 固有部分」の分離が要る。
...
- 壊れた JSON / 未知 key / `run.id` 不一致 / 未知 runtime / 空 model / `..` を含む prompt path のそれぞれで、
  launch が非 0 で止まり、manifest の絶対 path と理由が出る。DB 値への silent fallback が起きない。
