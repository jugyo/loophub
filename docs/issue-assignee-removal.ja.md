# issue assignee（`@lh-dev`）の廃止 — 設計判断

> Status: Decision（方針確定・実装は別 issue） · Issue: #180 · PR: #181
> 本書は「issue の assignee（`assignee_session_id` / UI の `@lh-dev` 表示）を**仕組みごと廃止する**」
...
issue 一覧・詳細に、紐づく dev session を `@lh-dev` のような assignee として表示している。
しかし今や `lh dev <issue>` するとすぐに **draft PR** が作られ、issue 詳細にも `linked PR #181`
...
> 注意: assignee は**表示専用ではない**。安易に列だけ消すと二重 `lh dev` ガードと
...
| issue の動機解消 | 達成（`@lh-dev` 表示は消える） | 達成 |
...
| assign 呼び出し | `cli/index.ts:535`（`lh dev`） / `cli/index.ts:803`（`lh issue assign`） / `web/server/contract.ts:220`（Web API） | assignee をセットする 3 経路 |

**確認した非自明な事実:**

- `lh dev <issue>` は assign（`cli/index.ts:535`）→ openPr（`cli/index.ts:554`）を claude 起動前に
  **同期**で連続実行する。draft PR は着手とほぼ同時に存在し、PR 無しの窓は通常フローでほぼゼロ。
- `createPull`（`core/store.ts:245`）は PR 行の `assignee_session_id` を**セットしない**。
  つまり `lh dev <issue>` フローの session 解決は **PR → `linkedIssue.assignee_session_id`** に
  完全依存している（PR 自身の assignee は `lh dev <pr>` で直接動かした場合のみ立つ）。
...
### 3.1 二重 `lh dev` ガード → open-PR 単位の hard 制約
...
- これにより「二重 `lh dev` を弾く」根拠が assignee の UNIQUE index から **open-PR 制約**へ移る。
...
- `lh dev` は openPr まで同期で進むため、PR 作成前に死ぬのは異常系（git/worktree 失敗等）。
...
- [ ] `lh dev` の assign 呼び出し（`cli/index.ts:535`）
...
4. **assign 経路を撤去**: `lh dev` / `lh issue assign` / Web エンドポイント。
...
issue assignee（`@lh-dev`）は draft PR と冗長になったため、**仕組みごと廃止**する。ただし
