...
> Status: Implemented · Issue: #975 / #981 / #1284 / #1307 / #1358 / #1555 / #1556 / #1680 / #1697 / #1712 / #1716
>
...
- **instruction は injection で配達する** — agent への入力は起動プロンプト、または生きている pane
  ...
  `lh workflow step status` のまま。event 到着待ちは agent runtime 管理の unified exec session で実行する
  blocking `lh workflow watch` が担い、完了結果を受け取った同じ親が処理を続ける。子は親の pane・topology を知らない。
...
| Execute → 親 | ターン完了の宣言（payload なし） | `lh workflow turn done` が event を記録（fact）。watcher exec session の完了結果から親が event を観測 |
...
| 世界 → 親 | turn done、workflow review 登録、GitHub PR feedback の観測 | blocking watcher が返す次の event。観測後に domain state を再確認 |
...
[Web / CLI]
  人間が確認済みの issue で workflow を開始
            ...
            ▼
  workflow agent（親）
    │  watcher exec session が返した turn_done / review_submitted / github_feedback の
    │  event を処理し、domain state を再観測して遷移する
    │
    ...
    └─ fresh pass review → run は running のまま watcher exec session を再実行
...
1. Execute 起動前に最新 run event id を cursor として取得し、起動後に
   `lh workflow watch --repo <repo> --run <run> --since <cursor> --json` を agent runtime 管理の unified exec session
   で実行する。次の run event が 1 件返ると完了結果を同じ parent が処理し、`lh workflow step status`
...
7. passing verdict 後も run と watcher exec session、および可能なら Execute pane を維持し、追加指示や
...
### Unified exec watcher protocol

Execute 起動前に run の最新 event id を取得する。event がなければ `cursor=0` とする。

```sh
lh events --repo "$repo" --type workflow_run --run "$run" --order desc --limit 1 --json
```
...
watcher の command 完了後、同じ parent が stdout の JSON result を回収する。non-zero exit は可視化し、
暗黙の retry や fallback を行わない。

JSON の `events` array は `cursor` より後の event を昇順で正確に 1 件含む。core service はその event id を
`next_since` として返し、CLI presentation layer が次の `--since` に設定した完全な `next_command` を JSON
へ追加する。各 transition は `lh workflow step status` が返す
HEAD / review 状態から決める。event を処理したら、親は `next_command` を編集せず次の `exec_command` の
`cmd` にそのまま渡し、必要なら同じ `session_id` / `write_stdin` 手順で待つ。CLI は cursor を DB に
永続化せず、acknowledgement も行わない。

parent が停止または cursor を失った場合、CLI は event を自動 replay しない。次の履歴と current step
status、参照された review / GitHub resource を確認して state を復元し、確認済みの最新 event id から再開する。

```sh
lh events --repo "$repo" --type workflow_run --run "$run" --order asc --json
...
deterministic test は event reader と待機を差し替え、空結果から 1 event への遷移、返却する
`next_command` の `--since` 境界、transition 後に既に存在する event の即時取得を検証する。
...
```text
since=<caller-cursor> repo=jugyo/loophub types=[workflow_run] runId=42 order=asc limit=1
```

CLI integration test は実 DB と別 process からの event 記録を使い、blocking watcher の終了、
JSON event、次回 watch command の生成、effect receipt の idempotence を確認する。不正引数と依存
failure は非0で可視化する。production agent runtime の watcher 完了後に同一 parent invocation が
続行する境界は、認証済み Claude Code または Codex を使う opt-in integration test で確認できる。Codex
path は 1 件目の event result を同じ parent が処理し、返された exact `next_command` を次の
`exec_command` に渡して 2 件目の event を受信するまでを検証する。generated parent prompt の test は
unified exec session、最終応答を出さない待機、完了 result の回収を検証する。
...
表示する。親は watcher が返した run-scoped event を処理する。CLI の filter は payload の `id`
...
|---|---|---|
| start | run started | 最新 event id を cursor に設定 → Execute を launch → watcher exec session を開始 |
...
fresh pass は現在の HEAD を検証するが、run を完了・凍結しない。親の watcher exec session と Execute pane を維持し、
...
| `core/service/workflow-watch.ts` | run-scoped blocking wait と次回 cursor（`next_since`）の解決 |
| `core/store/workflows.ts` | 2 prompt と workflow run / effect receipt の persistence |
| `core/serialize.ts` | workflow / run の wire shape |
| `cli/commands/workflow.ts` | `next_since` から `next_command` を組み立てる presentation を含む thin CLI |
...
- 親は turn done、review submitted、GitHub feedback の event を watcher exec session で 1 件ずつ処理し、
  返された `next_command` を次の待機に使う。その都度 domain state を再観測し、blocking wait 中に親 model
  turn は発生しない。
