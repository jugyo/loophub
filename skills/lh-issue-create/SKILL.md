...
on your own and do **not** ask. Implementation is started from the Web UI — the issue's **Build**
button or **Start workflow** control — not by this skill; no prompting from this skill is needed.
(Only implement in-skill if the user explicitly asked to both create and implement in the same
message.)
...
> Start working on this now? (opens a new herdr tab and starts the build)
...
- **Agrees**: launch the same flow the Web UI's **Build** button triggers — a new herdr tab +
  ...
  back to the builtin-mode guidance above (start the build from the Web UI) rather than leaving
  the user stuck.
...
**Builtin mode.** Do **not** auto-chain to implementation. After creating the issue, implementation
is started from the Web UI — the issue's **Build** button or **Start workflow** control. Only
implement inside this skill if the user explicitly asked to both create and implement in the same
message.
